...
class TLPApartmentInterior(HomeInterior):
    ...
    def check_interactions(self):
        ...
        # Near bed - pack items (works for both pack_belongings and pack_essentials)
        if self.is_near_position(self.bed_position) and ("Pack" in current_obj.description or current_obj.id == "pack_essentials"):
            if not self.items_packed or current_obj.id == "pack_essentials":  # Allow repacking for essentials
        ...
        # Near roommate - meet them
            ...
            # Start a dialogue cutscene
            self.start_roommate_dialogue()
    ...
    def start_roommate_dialogue(self):
        """Start the roommate meeting cutscene"""
        self.dialogue_active = True
        self.dialogue_index = 0
        self.dialogue_lines = [
            ("Alex", "Oh hey! You must be my new roommate!"),
            ("Alex", "I'm Alex. Nice to meet you!"),
            ("Alex", "I've been here for about a month now."),
            ("Alex", "The TLP program has been really helpful."),
            ("You", "Nice to meet you too! I just moved in."),
            ("Alex", "Cool! Let me know if you need anything."),
            ("Alex", "The kitchen's shared, and we take turns cleaning."),
            ("Alex", "Oh, and quiet hours are after 10 PM."),
            ("You", "Thanks for letting me know!"),
            ("Alex", "No problem! Welcome to the apartment!")
        ]
        self.dialogue_speaker = self.dialogue_lines[0][0]
    ...
    def draw_furniture(self, screen):
        ...
        # Highlight bed area if packing objective is active
        if current_obj and "Pack" in current_obj.description and not self.items_packed:
            bed_x = self.room_x + self.bed_position[0] * TILE_SIZE
            bed_y = self.room_y + self.bed_position[1] * TILE_SIZE
            # Draw glowing effect
            glow_surf = pygame.Surface((self.bed_width * TILE_SIZE + 20, self.bed_height * TILE_SIZE + 20), pygame.SRCALPHA)
            glow_alpha = int((math.sin(pygame.time.get_ticks() / 300.0) + 1) * 0.5 * 40 + 20)
            pygame.draw.rect(glow_surf, (120, 170, 255, glow_alpha), glow_surf.get_rect(), border_radius=10)
            screen.blit(glow_surf, (bed_x - 10, bed_y - 10))
    ...
    def draw_dialogue(self, screen):
        """Draw dialogue box for cutscenes"""
        if not self.dialogue_active or self.dialogue_index >= len(self.dialogue_lines):
            return
        ...
        pygame.draw.rect(screen, (70, 70, 80), (box_x, box_y, box_width, box_height), 3)
        
        # Get current dialogue
        speaker, text = self.dialogue_lines[self.dialogue_index]
        
        # Draw speaker name
        speaker_font = pygame.font.Font(None, 28)
        speaker_color = (120, 170, 255) if speaker == "Alex" else (255, 170, 120)
        speaker_surface = speaker_font.render(speaker + ":", True, speaker_color)
        screen.blit(speaker_surface, (box_x + 20, box_y + 20))
        
        # Draw dialogue text
        text_font = pygame.font.Font(None, 24)
        text_surface = text_font.render(text, True, (220, 220, 220))
        screen.blit(text_surface, (box_x + 20, box_y + 60))
        
        # Draw continue prompt
        prompt_font = pygame.font.Font(None, 20)
        prompt_text = "Press SPACE to continue" if self.dialogue_index < len(self.dialogue_lines) - 1 else "Press SPACE to finish"
        prompt_surface = prompt_font.render(prompt_text, True, (150, 150, 150))
        screen.blit(prompt_surface, (box_x + box_width - 200, box_y + box_height - 30))
