...
class GroceryStoreInterior(HomeInterior):
    ...
    def enter(self):
        ...
        # Check if we should launch the shopping mini-game
        current_obj = self.game.objective_manager.get_current_objective()
        if current_obj and current_obj.id == "grocery_shopping":
            # Launch the shopping mini-game instead of using the interior
            if hasattr(self.game.objective_manager, 'grocery_shopping'):
                print("Launching grocery shopping mini-game...")
                self.game.objective_manager.current_activity = self.game.objective_manager.grocery_shopping
                self.game.objective_manager.current_activity.start()
                # Exit the interior immediately
                self.exit()
                return
