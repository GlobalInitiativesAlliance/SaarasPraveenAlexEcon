...
class WaitlistTracker(Activity):
    ...
    def complete(self):
        ...
        if hasattr(self, 'narrative_ref'):
            self.narrative_ref.activity_completed('waitlist_tracker')
        self.game.current_activity = None
    ...
    def render(self, screen):
        ...
        if self.phase == 0:
            self.render_waitlist_position(screen)
        elif self.phase == 1:
            self.render_survival_timeline(screen)
        elif self.phase == 2:
            self.render_acceptance(screen)
            
    def render_waitlist_position(self, screen):
        """Phase 1: Show waitlist position"""
        center_x = SCREEN_WIDTH // 2
        center_y = SCREEN_HEIGHT // 2
        
        # Title
        title = self.game.font_large.render("TRANSITIONAL LIVING PROGRAM", True, self.text_color)
        title_rect = title.get_rect(center=(center_x, center_y - 200))
        screen.blit(title, title_rect)
        
        # Waitlist position - HUGE
        pos_text = self.game.font_large.render("WAITLIST POSITION", True, self.accent_color)
        ...
        # Wait time
        wait_text = self.game.font_medium.render(
        ...
        # The harsh reality
        reality_text = self.game.font_medium.render(
        ...
        # Continue prompt
        self.phase_complete = True
        prompt = self.game.font_medium.render("Press E to see survival timeline", True, self.success_color)
        prompt_rect = prompt.get_rect(center=(center_x, SCREEN_HEIGHT - 50))
        screen.blit(prompt, prompt_rect)
        
    def render_survival_timeline(self, screen):
        """Phase 2: Show 6 months of survival"""
        center_x = SCREEN_WIDTH // 2
        start_y = 100
        
        # Title
        title = self.game.font_large.render("6 MONTHS OF SURVIVAL", True, self.text_color)
        ...
        for i, (entry, color) in enumerate(self.survival_log):
            ...
            text_surface = self.game.font_medium.render(entry, True, color)
        ...
        if self.phase_complete:
            prompt = self.game.font_medium.render(
                "Press E to continue", 
    ...
    def render_acceptance(self, screen):
        """Phase 3: Finally accepted"""
        center_x = SCREEN_WIDTH // 2
        center_y = SCREEN_HEIGHT // 2
        
        # Success message
        success = self.game.font_large.render("ACCEPTANCE CALL!", True, self.success_color)
        success_rect = success.get_rect(center=(center_x, center_y - 100))
        screen.blit(success, success_rect)
        
        # Days survived
        days_text = self.game.font_medium.render(
        ...
        # Moving in message
        move_text = self.game.font_medium.render(
        ...
        for i, rule in enumerate(rules):
            rule_text = self.game.font_small.render(rule, True, self.text_color)
            rule_rect = rule_text.get_rect(center=(center_x, center_y + 120 + i * 30))
            screen.blit(rule_text, rule_rect)
            
        # Complete
        self.phase_complete = True
        prompt = self.game.font_medium.render("Press E to continue", True, self.success_color)
        prompt_rect = prompt.get_rect(center=(center_x, SCREEN_HEIGHT - 50))
        screen.blit(prompt, prompt_rect)