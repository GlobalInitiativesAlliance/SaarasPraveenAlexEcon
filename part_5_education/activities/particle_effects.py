"""
Particle Effects System for Education Mini-Games
...
class ParticleSystem:
    ...
    def emit_paper_trail(self, x: float, y: float, count: int = 5) -> None:
        """Emit paper-like particles for document movement"""
        if len(self.particles) >= self.max_particles:
            return
    ...
    def emit_typing_cursor(self, x: float, y: float) -> None:
        """Emit particles for typing/cursor effect"""
        if len(self.particles) >= self.max_particles:
            return
    ...
    def emit_stamp(self, x: float, y: float, color: Tuple[int, int, int] = (72, 187, 120)) -> None:
        """Emit particles for stamp effect"""
        if len(self.particles) >= self.max_particles:
            return
