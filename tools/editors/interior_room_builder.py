...
class InteriorRoomBuilder:
    def __init__(self):
        ...
        self.current_layer = 'floor'  # floor, walls, furniture, decor, characters
    ...
    def load_characters(self):
        """Load all premade character sprites"""
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(os.path.dirname(script_dir))

        base_path = os.path.join(project_root, "assets/moderninteriors-win/2_Characters/Character_Generator/0_Premade_Characters/32x32")

        for i in range(1, 21):  # Load characters 01 to 20
            filename = f"Premade_Character_32x32_{i:02d}.png"
            filepath = os.path.join(base_path, filename)

            try:
                spritesheet = pygame.image.load(filepath)
                # Extract idle frame (first frame)
                idle_frame = pygame.Surface((32, 32), pygame.SRCALPHA)
                idle_frame.blit(spritesheet, (0, 0), pygame.Rect(0, 0, 32, 32))

                self.characters.append({
                    'id': f"character_{i:02d}",
                    'name': f"Character {i}",
                    'sheet': spritesheet,
                    'idle': idle_frame
                })
    ...
    def draw_ui(self):
        ...
        # Draw tile palette or character palette based on current layer
        if self.current_layer == 'characters':
            self.draw_character_palette()
        else:
    ...
    def draw_character_palette(self):
        """Draw character selection palette when in character mode"""
        if self.current_layer != 'characters':
            return

        # Character palette position (where tileset viewer normally is)
        panel_x = self.tileset_x
        panel_y = self.tileset_y
        panel_width = 480
        panel_height = 600

        # Background
        pygame.draw.rect(self.screen, PANEL_COLOR, (panel_x, panel_y, panel_width, panel_height))
        pygame.draw.rect(self.screen, TEXT_COLOR, (panel_x, panel_y, panel_width, panel_height), 2)

        # Title
        title = self.font.render("CHARACTER SELECTION", True, TEXT_COLOR)
        self.screen.blit(title, (panel_x + 10, panel_y + 10))

        # Character grid (4 columns, 5 rows visible)
        cols = 4
        char_size = 48  # Display size for characters
        padding = 10
        ...
        visible_rows = 5
        start_idx = (self.character_scroll // char_size) * cols
        end_idx = min(start_idx + cols * visible_rows, len(self.characters))
        ...
        for i in range(start_idx, end_idx):
            ...
            x = start_x + col * (char_size + padding)
            y = start_y + row * (char_size + padding)

            # Character frame
            frame_rect = pygame.Rect(x, y, char_size, char_size)

            # Highlight selected
            if char == self.selected_character:
                pygame.draw.rect(self.screen, SELECTED_COLOR, frame_rect, 2)
            else:
                pygame.draw.rect(self.screen, GRID_COLOR, frame_rect, 1)

            # Draw character sprite (scaled up)
            if char['idle']:
                scaled = pygame.transform.scale(char['idle'], (char_size - 4, char_size - 4))
                self.screen.blit(scaled, (x + 2, y + 2))

            # Character name
            name_text = self.small_font.render(char['name'], True, TEXT_COLOR)
            self.screen.blit(name_text, (x, y + char_size + 2))

        # Scroll hint
        if len(self.characters) > cols * visible_rows:
        ...
        if (self.room_offset_x <= mouse_x < self.room_offset_x + room_pixel_width and
            ...
            if 0 <= hover_x < self.room_width and 0 <= hover_y < self.room_height:
                ...
                if self.current_tool == 'erase':
                    ...
                    # Preview tile or character
                    if self.current_tool == 'paint':
                        if self.current_layer == 'characters' and self.selected_character:
                            # Preview character
                            if self.selected_character.get('idle'):
                                preview_char = self.selected_character['idle'].copy()
                                preview_char.set_alpha(128)
                                self.screen.blit(preview_char, hover_rect)
        ...
        if self.current_tool == 'paint':
            if self.current_layer == 'characters':
                ...
                if self.selected_character:
                    self.room_layers['characters'][(x, y)] = self.selected_character.copy()
            else:
                if not self.selected_tile:
                    return
                self.room_layers[self.current_layer][y][x] = self.selected_tile
        elif self.current_tool == 'erase':
            # Erase based on layer type
            if self.current_layer == 'characters':
                # Remove character at position
                if (x, y) in self.room_layers['characters']:
                    del self.room_layers['characters'][(x, y)]
            else:
    ...
    def handle_input(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.button == 1:  # Left click
                ...
                if self.current_layer == 'characters' and mx >= self.tileset_x and my >= self.tileset_y:
                    # Character palette area
                    panel_x = self.tileset_x
                    panel_y = self.tileset_y

                    if panel_x <= mx <= panel_x + 480 and panel_y + 50 <= my <= panel_y + 600:
                        # Character grid area
                        cols = 4
                        char_size = 48
                        padding = 10

                        rel_x = mx - panel_x - padding
                        rel_y = my - panel_y - 50

                        col = rel_x // (char_size + padding)
                        row = rel_y // (char_size + padding)

                        if 0 <= col < cols:
                            visible_rows = 5
                            start_idx = (self.character_scroll // char_size) * cols
                            char_idx = start_idx + row * cols + col

                            if 0 <= char_idx < len(self.characters):
                                self.selected_character = self.characters[char_idx]
                                print(f"Selected: {self.selected_character['name']}")

                elif sheet and mx >= self.tileset_x and my >= self.tileset_y:
                    sheet_width_tiles = sheet.get_width() // ORIGINAL_TILE_SIZE
                    sheet_height_tiles = sheet.get_height() // ORIGINAL_TILE_SIZE
                    viewer_width = SCREEN_WIDTH - self.tileset_x - 10
                    viewer_height = SCREEN_HEIGHT - self.tileset_y - 10
            ...
            # Handle scrolling for character palette
            if self.current_layer == 'characters' and mouse_x >= self.tileset_x:
                # Scroll character palette
                char_size = 48
                self.character_scroll = max(0, min(1000, self.character_scroll - event.y * char_size))
            # Handle scrolling for saved rooms in left panel
            elif mouse_x < 280:  # Left panel area
                # Scroll saved rooms
